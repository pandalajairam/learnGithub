		...
		try{
			...
			if(env!=null && !(env.isEmpty()))
			{
				...
				if(env.equalsIgnoreCase("Dev")){
					...
					url = "http://" + getProperties().getProperty("prod_url").trim();
					suppressUrl = url;
				}else{
				...
				case "production":
					...
					commonProp.get().setProperty("urlSuppressCredentialSitecore", commonProp.get().getProperty("sitecore_url"));
				...
				case "dev":
					...
					commonProp.get().setProperty("urlSuppressCredential", suppressUrl);
					commonProp.get().setProperty("url", url);
					commonProp.get().setProperty("urlSuppressCredentialSitecore", commonProp.get().getProperty("urlSuppressCredentialSitecore"));
					commonProp.get().setProperty("seleniumGrid", String.valueOf(isGridExecution));
					commonProp.get().setProperty("cloudExecution", String.valueOf(isCloudExecution));
					commonProp.get().setProperty("localExecution", String.valueOf(isLocalExecution));
					commonProp.get().setProperty("deviceType", CommonVariables.DeviceName.get());
				...
				case "qa":
					...
					break;					
				case "stag":
					commonProp.get().setProperty("enviserName", commonProp.get().getProperty("dbUserName"));
					commonProp.get().setProperty("dbPronment", "STAG");
					commonProp.get().setProperty("dbServerName", commonProp.get().getProperty("dbServerName"));					
					commonProp.get().setProperty("dbUassword", commonProp.get().getProperty("dbPassword"));
					commonProp.get().setProperty("urlSuppressCredential", suppressUrl);
					commonProp.get().setProperty("url", url);
					commonProp.get().setProperty("urlSuppressCredentialSitecore", commonProp.get().getProperty("urlSuppressCredentialSitecore"));
					commonProp.get().setProperty("seleniumGrid", String.valueOf(isGridExecution));
				...
				System.out.println("Unable to find Environment type. It should be set at every Thread level under '@BeforeTest' method against 'CommonVariables.Environment' variable.");
				Assert.fail("Unable to find Environment type. It should be set at every Thread level under '@BeforeTest' method against 'CommonVariables.Environment' variable.");
			}
